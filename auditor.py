...
"""
# Auditoria determinista sobre los crons de una flota de agentes.
# Creado 2026-08-12 (L147). El auditor lo ejecuta cada noche: python3 <WORKSPACE>/workspace/audit_10d.py
# Salida: "LIMPIO" o lista de fallos. Un fallo = incidencia que el auditor corrige en el mismo run.
import re, sys, json, os

BASE = '<WORKSPACE>'
rb = open(f'{BASE}/workspace/Rulebook.md', encoding='utf-8').read()
...
for task in ['daily-run', 'outreach-run', 'nightly-audit']:
...
# Hasta hoy esta dimension exigia LO CONTRARIO: que E3-BIS.5 llevase CHANNEL_FLAG=true en los tres pases.
# Nacio el 12-08, cuando E3.0 prohibia los ATS externos y the single-step channel era el canal unico, asi que el
# invariante correcto entonces era "no te dejes CHANNEL_FLAG". El 19-08 OWNER derogo esa prohibicion (L191)
# y E3.0(f) ordeno explicitamente lanzar los pases SIN CHANNEL_FLAG y modernizar esta dimension. Durante ese
...
# derogada la prohibicion de ATS externos, E3-BIS.5 no puede seguir prescribiendo CHANNEL_FLAG en sus pases.
...
if e30 and p5:
    ...
    con_fal = [r for r in recetas if 'CHANNEL_FLAG' in r]
    if derogada and con_fal:
        fails.append(f"Rulebook: E3.0 deroga el canal unico pero E3-BIS.5 sigue prescribiendo CHANNEL_FLAG en {len(con_fal)} pase(s) -> {con_fal[0][:80]}")
    if not derogada and len(recetas) - len(con_fal) > 0:
        fails.append("Rulebook: E3.0 mantiene el canal unico the single-step channel pero E3-BIS.5 tiene pases sin CHANNEL_FLAG")
# 9 huerfanas del indice (exentas las solo-auditor declaradas)
rules = set(re.findall(r"^((?:[A-F]\d+(?:\.\d+)?(?:-[A-Z]+)?)|C0)\.", body, re.M))
work = idx.split("- **nightly-audit**")[0]
...
j = open(f'{BASE}/workspace/Journal.md', encoding='utf-8').read()
...
# 12 AMPLIFY TEST bandas de palabras del ultimo published item (25-40 o 60-75; prohibido 45-56)
led=open(f'{BASE}/workspace/Activity_Ledger.md',encoding='utf-8').read()
pubs=re.findall(r'PUBLISHED ITEM[^"]*"([^"]{40,})"',led)
if pubs:
    wc=len(pubs[-1].split())
...
# outreach-run, asi que cuando ese run leyo y ADEMAS reescribio F10 nadie lo detecto.
...
for line in idx.splitlines():
    m = re.match(r"\s*-\s+\*\*(daily-run|outreach-run)\*\*", line)
...
# the single-step channel (13-08, L150) y la del enlace de Drive (14-08, L153). Toda cita a "F12" resolvia a las dos
...
# Defecto que la origina: el daily del 18-08 declaro 19 resultados, envio 3 submissions y registro 15
# descartes. 3+15=18, y la tarjeta que faltaba (Head Of B2C, Madrid hibrido, the single-step channel) era elegible por
...
for m in re.finditer(r"^## (L\d+) \| (2026-\d\d-\d\d) \|([^\n]*)\n(.*?)(?=^## L|\Z)", j, re.M | re.S):
    ...
    if 'daily-run' not in titulo and 'daily-run' not in cuerpo[:2000]: continue
...
# submissions con las manos del sistema y NO escribe los ficheros de estado, asi que una submission
# suya es, para el dedupe de E3, una submission que nunca ocurrio. Medido el 20-08: siete correos con
...
# LIMITE DECLARADO (A1): esto comprueba que la submission esta REGISTRADA, nunca como esta escrito el
# correo; el cuerpo lo audita una persona con C0.6(d).
import glob, time
SESS = os.environ.get('SESSIONS_DIR', '<SESSIONS_DIR>')
def norm(s): return re.sub(r'[^a-z0-9]', '', (s or '').lower())
DIMENSIONES = 20  # snapshot publico; la version viva crece con cada incidencia
try:
    bl = json.load(open(f'{BASE}/workspace/Registry.json', encoding='utf-8'))
    CAMPANA_PUNTUAL_EXCLUIDA = tuple(os.environ.get('EXCLUDED_DOMAINS', '').split(','))
        ...
        for line in open(path, encoding='utf-8', errors='replace'):
            ...
            for c in (d.get('message', {}).get('content') or []):
                ...
                for dest in dests:
                    ...
                    # Estas entradas viven en su propio carril y no cuentan para este control.
                    ...
                    if not any(tok in k or (k and k in tok) for k in known if k):
                        fails.append(f"Candidatura enviada con CV a {dest} y SIN entrada en Registry.json (E3-QUINQUIES.a)")
...
try:
    ...
    diario = open(f'{BASE}/workspace/Journal.md', encoding='utf-8').read()
...
try:
    ...
    _diario = open(f'{BASE}/workspace/Journal.md', encoding='utf-8').read()
...
if fails:
    ...
    sys.exit(1)
print("LIMPIO: %d dimensiones" % DIMENSIONES)
